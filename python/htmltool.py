...
class Table:
	...
	def __init__(self, rows=None, border='1', style=None, width=None,
		...
		if style == None: self.style = Table.TABLE_STYLE_THINBORDER
		self.width	   = width
	...
	def __str__(self):
		...
		if self.col_width:
			for width in self.col_width:
				result += '  <COL width="%s">\n' % width
		n_cols = max(len(self.col_styles or []), len(self.col_width or []),
					 len(self.col_align or []), len(self.col_valign or []))
